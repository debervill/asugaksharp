...
| `SplitCommissionToLines(List<string>, int maxChars, int count)` | `string[]` | Раскладывает список имён по строкам через ", ", перенос при переполнении |
...
|------|-----|----------|----------|
| `fill_1` | "Протокол №" | номер протокола | `Diplomnik.ZasedanieOrder.ToString()` — порядковый номер студента в ГЭК; пусто если не задан |

### Дата заседания

| Поле | Alt | Значение | Источник |
|------|-----|----------|----------|
| `undefined` | — | день | `Diplomnik.Zasedanie.Date.Day.ToString()` |
| `undefined_2` | — | месяц | `RussianMonth(Diplomnik.Zasedanie.Date.Month)` |
| `undefined_3` | — | год | `Diplomnik.Zasedanie.Date.Year.ToString()` |
| `fill_5` | "г. с" | час начала | `timeStart.Hour` — автоматически: base 10:00, 15 мин × (ZasedanieOrder−1); **пусто если ZasedanieOrder не задан** |
| `fill_6` | "ч" | мин начала | `timeStart.Minute.ToString("D2")`; **пусто если ZasedanieOrder не задан** |
| `fill_7` | "мин. до" | час конца | `timeEnd.Hour` — base 10:00, 15 мин × ZasedanieOrder; **пусто если ZasedanieOrder не задан** |
| `fill_8` | "ч" | мин конца | `timeEnd.Minute.ToString("D2")`; **пусто если ZasedanieOrder не задан** |

> Формула времени: студент N → начало `10:00 + (N−1)×15 мин`, конец `10:00 + N×15 мин`.

### Студент

| Поле | Alt | Значение | Источник |
|------|-----|----------|----------|
| `fill_10` | "Обучающийся" | ФИО в именительном | `Diplomnik.FioImen` |
| `fill_9` | "Вид ВКР" | вид работы | `Diplomnik.VidVkr` |
| `fill_11` | "Направление подготовки" | название направления | `Diplomnik.ProfilPodgotovki.NapravleniePodgotovki.Nazvanie` |
| `fill_12` | "Профиль/специализация" | первая часть профиля | `SplitAtWidth(ProfilPodgotovki.Name, 45).first` |

### Тема ВКР

| Поле | Alt | Значение | Источник |
|------|-----|----------|----------|
| `Text1` | — | остаток профиля | `SplitAtWidth(ProfilPodgotovki.Name, 45).rest` |
...
| Поле | Alt | Значение | Источник |
|------|-----|----------|----------|
| `fill_13` | "Руководитель" | ФИО руководителя | `Diplomnik.Person.Name` |
...
| Поле | Alt | Значение | Источник |
|------|-----|----------|----------|
| `fill_16_2` | "Признать… с оценкой" | оценка | `Diplomnik.Otsenka` |
| `fill_17_2` | "ФИО" | краткое ФИО председателя | `ShortName(Gak.Predsedatel.Name)` |
| `fill_18_2` | "подпись председателя" | — | **не заполняется** — оставить пустым для ручной подписи |
| `fill_19_2` | "ФИО секретаря" | — | **не заполняется** — оставить пустым для ручной подписи |
...
| `Text12` | профиль/специализация (продолжение) | `SplitAtWidth(ProfilPodgotovki.Name, 45).rest` |
| `Text13` | вид ВКР | `Diplomnik.VidVkr` |

### Комиссия (Text14–Text18)

`Text14` — **только председатель**. `Text15–Text18` — остальные члены комиссии (без председателя и секретаря), перечисляются через ", ", перенос при переполнении (~65 символов).

| Поле | Значение | Источник |
|------|----------|----------|
| `Text14` | председатель | `Diplomnik.Zasedanie.Gak.Predsedatel.Name` |
| `Text15` | члены комиссии, строка 1 | `SplitCommissionToLines(members, 65, 4)[0]` |
| `Text16` | члены комиссии, строка 2 | `...[1]` |
| `Text17` | члены комиссии, строка 3 | `...[2]` |
| `Text18` | члены комиссии, строка 4 | `...[3]` |
...
| `Text21` | квалификация | `Diplomnik.Zasedanie.Kvalificacia` |

### Подписи

| Поле | Alt | Значение |
|------|-----|----------|
| `fill_2` | "ФИО" | **не заполняется** — оставить пустым для ручной подписи председателя |
| `fill_4` | "ФИО" | **не заполняется** — оставить пустым для ручной подписи секретаря |
